...
const VideoPreview = forwardRef<VideoPreviewHandle, VideoPreviewProps>(({
  ...
  return (
    ...
    >
          ...
          return (
            <video
              ...
              className={`absolute inset-0 w-full h-full ${videoFitClass} cursor-grab active:cursor-grabbing ${
                isSelected ? 'ring-2 ring-yellow-500 ring-offset-2 ring-offset-black' : ''
              }`}
              style={styles}
              playsInline
              preload="auto"
          ...
          if (isOverlay) {
            ...
            return (
              ...
              >
                ...
                {/* Selection indicator */}
                {isSelected && (
                  <div className="absolute inset-0 ring-2 ring-yellow-500 rounded-lg pointer-events-none" />
                )}
      ...
      {/* Dragging indicator */}
      {draggingLayer && (
        <div className="absolute bottom-3 left-3 text-xs text-yellow-400 bg-black/70 px-2 py-1 rounded z-50">
          Dragging...
        </div>
      )}
    </div>
